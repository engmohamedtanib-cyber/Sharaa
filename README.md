...
The **deterministic core** is implemented and tested here, with no external
...
| `validation/` (V1–V9) | ✅ pure, deterministic |
| `engine/` (screens A–E, 7-pillar scoring, decisions, watchlist, portfolio, purification) | ✅ pure, deterministic |
| `research/`, `ingestion/`, `reporting/` | ⏳ interfaces + stubs (need Supabase / Anthropic / live data to run) |
...
uv run egx --help            # CLI
uv run pytest                # engine + validation tests
uv run ruff check .
uv run mypy src/engine src/validation
```
